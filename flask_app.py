...
df = pd.read_csv("flask_data.csv", sep=',')
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            dcc.Input(id="prompt-length", type="number", value=750, min=0, step=1, debounce=True, style={"width": "120px"})
        ], style={"display": "flex", "alignItems": "center", "marginBottom": "15px"}),

        html.Div([
            html.Label("Transaction history size (# chars):", style={"marginRight": "10px"}),
            dcc.Input(id="input-words", type="number", value=750, min=0, step=1, debounce=True, style={"width": "120px"})
        ], style={"display": "flex", "alignItems": "center", "marginBottom": "15px"}),

        html.Div([
            html.Label("Output Length (words):", style={"marginRight": "10px"}),
            dcc.Input(id="output-words", type="number", value=150, min=0, step=1, debounce=True, style={"width": "120px"})
        ], style={"display": "flex", "alignItems": "center", "marginBottom": "15px"}),

        html.Div([
            html.Label("Number of Runs:", style={"marginRight": "10px"}),
            dcc.Input(id="runs", type="number", value=1, min=1, step=1, debounce=True, style={"width": "120px"})
        ], style={"display": "flex", "alignItems": "center", "marginBottom": "15px"}),

        html.Div([
            html.Label("Number of Customers:", style={"marginRight": "10px"}),
            dcc.Input(id="customers", type="number", value=1, min=1, step=1, debounce=True, style={"width": "120px"})
        ], style={"display": "flex", "alignItems": "center", "marginBottom": "15px"}),
...
def update_cost_chart(selected_providers, prompt_length, input_words, output_words, runs, customers, currency):
    ...
    fig = px.bar(
        ...
        y="total_cost",
        color="provider",
        title=f"Total Cost ({input_words} input chars, {output_words} output words) in {y_label}",
